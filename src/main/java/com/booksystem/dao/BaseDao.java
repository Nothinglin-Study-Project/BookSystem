 ...
 * 更新操作（添加、修改、删除）
 * 查询操作
 * 1.查询一个字段（只返回一条记录且只有一个字段，一般用于查询总数量）
 * 2.查询集合
 * 3.查询某个对象
 */
public class BaseDao {
    ...
    /**
     * 查询一个字段（只会返回一条记录且只有一个字段，一般查询总数量
     * 1.得到数据库连接
     * 2.定义sql语句
     * 3.预编译
     * 4.如果有参数，则设置参数，下标从1开始
    ...
    public static Object findSingleValue(String sql, List<Object> params) {
        ...
        try {
            ...
            //判断并分析结果集,如果结果集不为空，则赋值给object变量进行返回结果
            if (resultSet.next()) {
    ...
    /**
     * 查询集合(要求JavaBean中的字段与数据库中表的字段对应)
     * 1。得到数据库连接
     * 2.定义sql语句
     * 3.预编译
     * 4.如果有参数，则设置参数，下标从1开始（数组或集合、循环设置参数）
     * 5.执行查询，得到结果集
     * 6.得到结果集的元数据对象（查询到的字段数量以及查询到哪些字段）
     * 7.判断并分析结果集
     * 8.实例化对象
     * 9.遍历查询的字段数量，得到数据库中查询到每一个列名
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                for (int i = 1; i <= fieldNum; i++) {
                    //得到查询的每一个列名
                    //getColumnLabel()：获得列名或别名
                    //getColumnName()：获得列名
                    String columnName = resultSetMetaData.getColumnLabel(i);
                    //通过反射，使用列名得到对应field对象
    ...
    public static List findAll(String sql, Class cls) {
        List list = new ArrayList();
        Connection connection = null;
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
